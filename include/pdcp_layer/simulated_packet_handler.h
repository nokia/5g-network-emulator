...
public:
    ...
    bool inject_bits(float bits, std::uint32_t tag) override;
...
private:
    ...
    void packetize(float bits, float current_t, std::uint32_t tag);
...
private:
    ...
    std::vector<std::pair<std::uint32_t, float>> pending_injections_;
